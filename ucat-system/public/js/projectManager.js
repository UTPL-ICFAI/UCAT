...
async function loadProjectTasks() {
  try {
    ...
    document.getElementById('tasksTableBody').innerHTML = html || '<tr><td colspan="5">No tasks yet</td></tr>';
    ...
    document.getElementById('tasksProgress').style.width = allTasks.length ? (completed / allTasks.length * 100) + '%' : '0%';
  } catch (error) {
...
async function loadProjectImages() {
  try {
    const response = await fetch(`/api/images?project_id=${currentProjectId}&status=approved`);
    ...
    if (allImages.length === 0) {
      gallery.innerHTML = '<div style="grid-column: 1/-1; text-align: center; padding: 40px; color: #999;">No approved images yet</div>';
    }
  } catch (error) {
...
async function loadProjectDocuments() {
  try {
    const response = await fetch(`/api/documents?project_id=${currentProjectId}`);
...
async function loadProjectIssues() {
  try {
    ...
    document.getElementById('issuesTableBody').innerHTML = html || '<tr><td colspan="5">No issues</td></tr>';
  } catch (error) {
...
function displayCommunications() {
  ...
  chatDiv.innerHTML = allCommunications.reverse().map(msg => `
...
async function loadBudgetTracking() {
  try {
    ...
    const spent = budgetData.reduce((sum, item) => sum + parseFloat(item.amount_spent), 0);
    ...
    document.getElementById('remainingAmount').textContent = '₹' + (total - spent).toFixed(2);
    document.getElementById('budgetProgress').style.width = total ? (spent / total * 100) + '%' : '0%';
...
async function loadTeamInfo() {
  try {
    ...
    document.getElementById('teamList').innerHTML = html || '<li>No team members</li>';
  } catch (error) {
...
function exportDocsCSV() {
  const rows = [['Document', 'Drawing No', 'Status', 'Category', 'Uploaded By', 'Date']];
  allDocuments.forEach(doc => {
    rows.push([doc.title || 'Untitled', doc.drawing_no || '-', doc.doc_status || '-', doc.category || '-', doc.uploaded_by_name || '-', formatDateShort(doc.created_at)]);
  });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('uploadDocForm').addEventListener('submit', handleUploadDoc);
  document.getElementById('projectSearch').addEventListener('keyup', updateProjectSidebar);
