...
function loadProjectsForSubmissions() {
  ...
  fetch("/api/projects", {
    ...
    .then((data) => {
      // FIX: /api/projects returns a plain array, not { success, data }
...
function loadTemplatesForProject() {
  ...
  fetch(`/api/project-templates/${projectId}`, {
    ...
    .then((response) => response.json())
    .then((data) => {
      // FIX A: API returns { success, projectTemplates }, not { success, data }
      // FIX B: template fields (name, fields, rows) are top-level on each item,
      //        NOT nested under an .template property — build the template object manually
      if (data.success && Array.isArray(data.projectTemplates)) {
        ...
        data.projectTemplates.forEach((assignment) => {
          ...
          option.dataset.templateData = JSON.stringify(templateObj);
  ...
  fetch(`/api/project-templates/submissions/${submissionId}`, {
    ...
    .then((data) => {
      if (data.success) {
        ...
        html +=
          '<h4>Submitted Data:</h4><div style="background: #f5f5f5; padding: 15px; border-radius: 6px;"><pre style="margin: 0;">';
