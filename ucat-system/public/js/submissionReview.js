...
function loadPMSubmissions() {
  // Get currentProjectId from global variable in projectManager.js
      ...
      if (data.success && Array.isArray(data.data)) {
        ...
        if (templateFilter) {
          submissions = submissions.filter(
            (s) => s.template_id == templateFilter,
          );
        }
        if (statusFilter) {
        ...
        loadTemplatesForFilter(projectId);
      }
    })
 ...
 */
function loadTemplatesForFilter(projectId) {
  // Use projectId parameter or fall back to global
  const pId = projectId || window.currentProjectId;
  if (!pId) return;

  const token = localStorage.getItem("auth_token");

  fetch(`/api/project-templates/${pId}`, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  })
...
function approveSubmission() {
  ...
  fetch(
    `/api/project-templates/submissions/${currentReviewSubmission.id}/approve`,
    {
      method: "POST",
      headers: {
...
function confirmRejection() {
  ...
  fetch(
    `/api/project-templates/submissions/${currentReviewSubmission.id}/reject`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
function generateSubmissionDocument(submissionId) {
  ...
  fetch(
    `/api/project-templates/submissions/${submissionId}/generate-document`,
    {
      headers: {
        Authorization: `Bearer ${token}`,
      },
