 ...
 */
router.post('/users', authenticate, requireRole(['superadmin']), async (req, res) => {
  ...
  try {
    ...
    const userResult = await client.query(
      ...
      [name, age, gender || null, employment_id, role, user_id, hashedPassword]
    );
    ...
    const defaultPermissions = {
      ...
      project_manager: { can_view: true, can_edit: true, can_delete: false, can_manage_users: true, can_manage_projects: false },
      // Site engineers can view, edit, manage projects
      site_engineer: { can_view: true, can_edit: true, can_delete: false, can_manage_users: false, can_manage_projects: true },
      // Supervisors can view and manage projects
      supervisor: { can_view: true, can_edit: false, can_delete: false, can_manage_users: false, can_manage_projects: true }
    };
    // Get default permissions for this user's role
    const perms = defaultPermissions[role] || { can_view: true, can_edit: false, can_delete: false, can_manage_users: false, can_manage_projects: false };
    ...
    await client.query(
      ...
      [newUserId, perms.can_view, perms.can_edit, perms.can_delete, perms.can_manage_users, perms.can_manage_projects]
    );
  ...
  try {
    ...
    if (employment_id !== undefined) {
      ...
      if (existingEmp.rows.length > 0) {
        ...
        await client.query('ROLLBACK');
        // Return 400 error for duplicate employment ID
        return res.status(400).json({ error: 'Employment ID already in use' });
      }
    }
...
router.get('/stats', authenticate, requireRole(['superadmin']), async (req, res) => {
  try {
    ...
    const documentsResult = await db.query(
      // SQL query to count all documents
 ...
 */
router.get('/permissions/all', authenticate, requireRole(['superadmin']), async (req, res) => {
  try {
 ...
 */
router.get('/activity', authenticate, requireRole(['superadmin']), async (req, res) => {
  try {
