...
CREATE INDEX idx_expense_entries_expense_date ON expense_entries(expense_date);
...
-- Create budget extension requests table
CREATE TABLE budget_extension_requests (
  id SERIAL PRIMARY KEY,
  project_id INTEGER REFERENCES projects(id) ON DELETE CASCADE,
...
CREATE INDEX idx_budget_extension_requests_project_id ON budget_extension_requests(project_id);
CREATE INDEX idx_budget_extension_requests_status ON budget_extension_requests(status);
CREATE INDEX idx_budget_extension_requests_requested_by ON budget_extension_requests(requested_by);

-- Track budget alert notifications per project
